...
class WxPayHelper
{
    ...
    public $partnerkey = null;
    public $appsecret = null;

	var $parameters; //cft 参数
	function __construct($appid,$appkey,$signtype,$partnerkey,$appsecret)
	{
        $this->appid= $appid;
        $this->appkey= $appkey;
	...
	protected function get_biz_sign($bizObj){
		 ...
		 try {
		 	...
		 	$bizParameters["$this->appkey"] = $this->appkey;
		 	ksort($bizParameters);
	...
	*/
	function create_app_package($traceid=""){
        ...
        try {
		   ...
		   if($this->check_cft_parameters() == false) {
		    ...
		    $nativeObj["$this->appid"] = $this->appid;
	...
	//生成jsapi支付请求json
	/*
	"$this->appid" : "wxf8b4f85f3a794e77", //公众号名称，由商户传入
	...
	//扩展字段，由商户传入
	"$this->signtype" : "SHA1", //微信签名方式:sha1
	...
	*/
	function create_biz_package(){
		 try {
		    ...
		    $nativeObj["$this->signtype"] = $this->signtype;
	...
	//生成原生支付url
	/*
	weixin://wxpay/bizpayurl?sign=XXXXX&$this->appid=XXXXXX&productid=XXXXXX&timestamp=XXXXXX&noncestr=XXXXXX
	*/
	...
	/*
	<xml>
    <$this->appid><![CDATA[wwwwb4f85f3a797777]]></$this->appid>
