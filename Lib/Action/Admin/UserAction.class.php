            ...
            if($UserDB->create()){
                ...
                if($user_id){
                    ...
                    if (M("RoleUser")->data($data)->add()){
                        $this->success('添加成功！','index.php?g=Admin&m=User&a=index');
                    }else{
                        $this->error('用户添加成功,但角色对应关系添加失败!','index.php?g=Admin&m=User&a=index');
                    }
                }else{
                     $this->error('添加失败!');
                }
        ...
        if(isset($_POST['dosubmit'])) {
            ...
            if($UserDB->create()){
                if($UserDB->save()){
                    ...
                    $this->success('编辑成功！','index.php?g=Admin&m=User&a=index');
                }else{
                     $this->error('编辑失败!','index.php?g=Admin&m=User&a=index');
                }
            }else{
                $this->error($UserDB->getError());
            }
    ...
    public function del(){
        ...
        if($UserDB->delUser('id='.$id)){
            if(M("RoleUser")->where('user_id='.$id)->delete()){
                ...
                $this->success('删除成功！','index.php?g=Admin&m=User&a=index');
            }else{
