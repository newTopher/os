...
class MapAction extends Action{
	...
	public function staticCompanyMap(){
		...
		if ($companies){
			...
			foreach ($companies as $thisCompany){
				...
				$return[]=array($i.'.'.$thisCompany['name'].'地图',"电话：".$thisCompany['tel']."\r\n地址：".$thisCompany['address']."\r\n点击查看详细",$imgUrl,C('site_url').'/index.php?g=Wap&m=Company&a=drive&longitude='.$thisCompany['longitude'].'&latitude='.$thisCompany['latitude'].'&companyid='.$thisCompany['id'].'&token='.$this->token);
				$i++;
			}
			//使用操作
	...
	public function walk($x,$y,$companyid=1){
		...
		if (is_array($rt)){
			...
			if ($distance>500){
				$distanceStr=(round($distance/500,2)).'公里';
			}else {
