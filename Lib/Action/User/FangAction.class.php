...
class FangAction extends BaseAction{
    ...
    public function index(){
        ...
        if(IS_POST){
            $_POST['lou_info'] = stripslashes(htmlspecialchars_decode($_POST['lou_info']));
            $_POST['jiao_info'] = stripslashes(htmlspecialchars_decode($_POST['jiao_info']));
            $_POST['hu_info'] = stripslashes(htmlspecialchars_decode($_POST['hu_info']));
            $id=$_POST['id'];
            ...
            if($this->Fang_model->where($where)->save($_POST)){
                ...
                $this->success2('修改成功!',U('Fang/index',array('token'=>$this->token)));
            }else{
                ...
                if(empty($check)){
                    ...
                    $this->success2('添加成功!',U('Fang/index',array('token'=>$this->token)));
                }else{
                    $this->error2('操作失败');
                }
                //echo '0022';die;
    ...
    public function add(){
        ...
        if(IS_POST){
            if($this->Fang_zi->add($_POST)){
                $this->success2('添加成功！',U('Fang/zindex',array('token'=>$this->token)));
            }else{
                $this->error2('添加失败！');
            }
        }else{
    ...
    public function del(){
        ...
        if(IS_GET){
            ...
            if($back==true){
                $this->success2('操作成功',U('Fang/zindex',array('token'=>$this->token)));
            }else{
                $this->error2('服务器繁忙,请稍后再试',U('Fang/index',array('token'=>$this->token)));
            }
    ...
    public function set(){
        ...
        if(empty($checkdata)||$checkdata['token']!=$this->token){
            $this->error2("没有相应记录.您现在可以添加.",U('Fang/zadd',array('token'=>$this->token)));
        }
        if(IS_POST){
            ...
            if($this->Fang_zi->create()){
                if($this->Fang_zi->where($where)->save($_POST)){
                    $this->success2('修改成功',U('Fang/zindex',array('token'=>$this->token)));
                }else{
                    $this->error2('操作失败');
                }
            }else{
                $this->error2($this->Fang_zi->geterror2());
            }
        }else{
    ...
    public function hu_add(){
        ...
        if(IS_POST){
            ...
            if($this->Fang_hu->add($_POST)){
                $this->success2('添加成功！',U('Fang/hu_index',array('token'=>$this->token)));
            }else{
                $this->error2('添加失败！');
            }
        }else{
    ...
    public function hu_set(){
        ...
        $this->assign('arr',$arr);
        if(empty($checkdata)||$checkdata['token']!=$this->token){
            $this->error2("没有相应记录.您现在可以添加.",U('Fang/hu_add'));
        }
        if(IS_POST){
            $_POST['info'] = stripslashes(htmlspecialchars_decode($_POST['info']));
            $where=array('id'=>$this->_post('id'),'token'=>$this->token);
            $check=$this->Fang_hu->where($where)->find();
            if($check==false)$this->error2('非法操作');
            if($this->Fang_hu->create()){
                if($this->Fang_hu->where($where)->save($_POST)){
                    $this->success2('修改成功',U('Fang/hu_index',array('token'=>$this->token)));
                }else{
                    $this->error2('操作失败');
                }
            }else{
                $this->error2($this->Fang_hu->geterror2());
            }
        }else{
            $su=array(1,2,3,4,5);
            $this->assign('su',$su);
    ...
    public function hu_del(){
        ...
        if(IS_GET){
            ...
            if($back==true){
                $this->success2('操作成功',U('Fang/hu_index',array('token'=>$this->token)));
            }else{
                $this->error2('服务器繁忙,请稍后再试',U('Fang/hu_index',array('token'=>$this->token)));
            }
    ...
    public function dianping_add(){
        ...
        if(IS_POST){
            if($this->Fang_dianping->add($_POST)){
                $this->success2('添加成功！',U('Fang/dianping_index',array('token'=>$this->token)));
            }else{
                $this->error2('添加失败！');
            }
        }else{
    ...
    public function dianping_set(){
        ...
        if(empty($checkdata)||$checkdata['token']!=$this->token){
            $this->error2("没有相应记录.您现在可以添加.",U('Fang/dianping_add'));
        }
        if(IS_POST){
            $where=array('id'=>$this->_post('id'),'token'=>$this->token);
            ...
            if($this->Fang_dianping->create()){
                if($this->Fang_dianping->where($where)->save($_POST)){
                    $this->success2('修改成功',U('Fang/dianping_index',array('token'=>$this->token)));
                }else{
                    $this->error2('操作失败');
                }
            }else{
                $this->error2($this->Fang_dianping->geterror2());
            }
        }else{
    ...
    public function dianping_del(){
        ...
        if(IS_GET){
            ...
            if($back==true){
                $this->success2('操作成功',U('Fang/dianping_index',array('token'=>$this->token)));
            }else{
                $this->error2('服务器繁忙,请稍后再试',U('Fang/dianping_index',array('token'=>$this->token)));
            }
    ...
    public function effect_add(){
        ...
        if(IS_POST){
            if($this->Fang_effect->add($_POST)){
                $this->success2('添加成功！',U('Fang/effect_index',array('token'=>$this->token)));
            }else{
                $this->error2('添加失败！');
            }
        }else{
    ...
    public function effect_set(){
        ...
        if(empty($checkdata)||$checkdata['token']!=$this->token){
            $this->error2("没有相应记录.您现在可以添加.",U('Fang/effect_add'));
        }
        if(IS_POST){
            $where=array('id'=>$this->_post('id'),'token'=>$this->token);
            ...
            if($this->Fang_effect->create()){
                if($this->Fang_effect->where($where)->save($_POST)){
                    $this->success2('修改成功',U('Fang/effect_index',array('token'=>$this->token)));
                }else{
                    $this->error2('操作失败');
                }
            }else{
                $this->error2($this->Fang_effect->geterror2());
            }
        }else{
    ...
    public function effect_del(){
        ...
        if(IS_GET){
            ...
            if($back==true){
                $this->success2('操作成功',U('Fang/effect_index',array('token'=>$this->token)));
            }else{
                $this->error2('服务器繁忙,请稍后再试',U('Fang/effect_index',array('token'=>$this->token)));
            }
    ...
    public function photo_add(){
        ...
        if(IS_POST){
            if($this->Fang_photo->add($_POST)){
                $this->success2('添加成功！',U('Fang/photo_index',array('token'=>$this->token)));
            }else{
                $this->error2('添加失败！');
            }
        }else{
    ...
    public function photo_set(){
        ...
        if(empty($checkdata)||$checkdata['token']!=$this->token){
            $this->error2("没有相应记录.您现在可以添加.",U('Fang/photo_add'));
        }
        if(IS_POST){
            $where=array('id'=>$this->_post('id'),'token'=>$this->token);
            ...
            if($this->Fang_photo->create()){
                if($this->Fang_photo->where($where)->save($_POST)){
                    $this->success2('修改成功',U('Fang/photo_index',array('token'=>$this->token)));
                }else{
                    $this->error2('操作失败');
                }
            }else{
                $this->error2($this->Fang_photo->geterror2());
            }
        }else{
    ...
    public function photo_del(){
        ...
        if(IS_GET){
            ...
            if($back==true){
                $this->success2('操作成功',U('Fang/photo_index',array('token'=>$this->token)));
            }else{
                $this->error2('服务器繁忙,请稍后再试',U('Fang/photo_index',array('token'=>$this->token)));
            }
    ...
    public function yuyue_add(){
        ...
        if(IS_POST){
            ...
            if($id = $this->Fang_yuyue->add($_POST)){
                ...
                $keyword_model->add($key);
                $this->success2('添加成功！',U($this->type.'/yuyue_index',array('token'=>$this->token)));
            }else{
                $this->error2('添加失败！');
            }
        }else{
    ...
    public function yuyue_set(){
        ...
        if(empty($checkdata)||$checkdata['token']!=$this->token){
            $this->error2("没有相应记录.您现在可以添加.",U($this->type.'/add'));
        }
        $lbs=M("Company")->where(array('token'=>$this->token))->select();
        $arr=array();
        foreach($lbs as $v){
        ...
        if(IS_POST){
            ...
            if($this->Fang_yuyue->create()){
                ...
                if($this->Fang_yuyue->where($where)->save($_POST)){
                    $this->success2('修改成功',U($this->type.'/yuyue_index',array('token'=>$this->token,'pid'=>$this->_get('pid'))));
                    $keyword_model=M('Keyword');
                ...
                $this->error2($this->Fang_yuyue->geterror2());
            }
        }else{
            $this->assign('isUpdate',1);
            $this->assign('set',$checkdata);
    ...
    public function yuyue_del(){
        ...
        if(IS_GET){
            ...
            if($back==true){
                ...
                $this->success2('操作成功',U($this->type.'/yuyue_index',array('token'=>$this->token)));
            }else{
                $this->error2('服务器繁忙,请稍后再试',U($this->type.'/yuyue_index',array('token'=>$this->token)));
            }
        }
    }
    //订单列表显示
    ...
    public function yuyue_setinfo(){
        ...
        if($check != 0 ){
            ...
            $this->success2('修改成功！',U($this->type.'/yuyue_index',array('token'=>$this->token)));die;
        }
        $this->assign('type',$this->type);
        $this->display();
    }
