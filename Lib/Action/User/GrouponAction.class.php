...
class GrouponAction extends UserAction{
	...
	public function add(){ 
		if(IS_POST){
            $_POST['endtime']=strtotime($this->_post('endtime')." 23:59:59");
		...
		if(IS_POST){ 
			...
			if($this->product_model->create()){
				if($this->product_model->where($where)->save($_POST)){
					...
					$this->success2('修改成功',U('Groupon/products',array('token'=>$this->token)));
				}else{
					$this->error2('操作失败');
				}
			}else{
				$this->error2($this->product_model->geterror2());
			}
		}else{
			//分类
			$catWhere=array('parentid'=>0,'token'=>$this->token);
