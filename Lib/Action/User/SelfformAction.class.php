...
class SelfformAction extends UserAction{
	...
	public function _initialize() {
		...
		/*if(!strpos($token_open['queryname'],'Selfform_set')){
            	$this->error2('您未开启该模块的使用权,请到功能模块中添加',U('Function/index',array('token'=>session('token'),'id'=>session('wxid'))));
		}*/
	...
	public function index(){
		if (IS_POST){
			...
			$this->success2('操作成功',U('Selfform/index',array('token'=>$this->token)));
		}else{
	...
	public function set(){
		...
		if(empty($checkdata)||$checkdata['token']!=$this->token){
            $this->error2("没有相应记录.您现在可以添加.",U('Selfform/add'));
        }
		if(IS_POST){ 
			...
			if($this->selfform_model->create()){
				if($this->selfform_model->where($where)->save($_POST)){
					$this->success2('修改成功',U('Selfform/index',array('token'=>$this->token)));
				...
				$this->error2($this->selfform_model->geterror2());
			}
		}else{
        ...
        if(IS_GET){                              
            ...
            if($back==true){
                ...
                $this->success2('操作成功',U('Selfform/index',array('token'=>$this->token)));
                 ...
                 $this->error2('服务器繁忙,请稍后再试',U('Selfform/index',array('token'=>$this->token)));
            }
        }        
	}
	...
	public function inputSet(){
		...
		if(empty($checkdata)){
            $this->error2("没有相应记录.您现在可以添加.",U('Selfform/inputAdd'));
        }
		if(IS_POST){
			...
			$rt=$this->selfform_input_model->where($where)->save($_POST);
				if($rt){
					$this->success2('修改成功',U('Selfform/inputs',array('token'=>$this->token,'id'=>$thisForm['id'])));
				}else{
					$this->error2('操作失败');
				}
			}else{
				$this->error2($this->selfform_input_model->geterror2());
			}
		}else{
			$this->assign('isUpdate',1);
	...
	public function inputDelete(){
		...
		if($back==true){
			$this->success2('操作成功',U('Selfform/inputs',array('token'=>$this->token,'id'=>$thisForm['id'])));
		}else{
			$this->error2('服务器繁忙,请稍后再试',U('Selfform/inputs',array('token'=>$this->token,'id'=>$thisForm['id'])));
		}
	}
	//
	...
	public function infoDelete(){
		...
		if($back==true){
			$this->success2('操作成功',U('Selfform/infos',array('token'=>$this->token,'id'=>$thisFrom['id'])));
		}else{
			$this->error2('服务器繁忙,请稍后再试',U('Selfform/infos',array('token'=>$this->token,'id'=>$thisFrom['id'])));
		}
	}
}
