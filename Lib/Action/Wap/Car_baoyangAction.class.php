...
$data['status']  =  "未处理";
 ...
 $count = M('Car_baoyang_input')->where(array('token'=>$data['token'],'wecha_id'=>$data['wecha_id'],'status'=>0,'hid'=>$data['hid']))->count();

if ($count<1) $order = M('Car_baoyang_input')->data($data)->add();       
