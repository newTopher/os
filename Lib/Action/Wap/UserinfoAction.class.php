...
class UserinfoAction extends BaseAction{
			...
			if($cardinfo){ //如果Member_card_create 不为空，说明领过卡，但是可以修改会员信息
				...
				//如果商家还有会员卡，可以领
					if($card != false){
						...
						//商家没有了会员卡
						$token=M('Wxuser')->field('uid')->where(array('token'=>$data['token']))->find();
	...
	public function cardrec(){
		...
		$sql=D('member_card_sign');
		$cardrec1=$sql->where(array('wecha_id'=>$this->_get('wecha_id'),'token'=>$this->_get('token'),'add_Balance' => array('gt',0)))->select();
		
		$cardrec2=$sql->where(array('wecha_id'=>$this->_get('wecha_id'),'token'=>$this->_get('token'),'sell_expense'=> array('gt',0)))->select();
		...
		$this->assign('cardnum',$cardinfo['number']);
		$this->assign('homepic',$img);
		$this->assign('cardrec1',$cardrec1);
		$this->assign('cardrec2',$cardrec2);
