$(function(){
	$('#show_captcha img').attr('src', '/reg/captcha?f=business_reg&tmp='+Math.random()).click(function(b){this.src='/reg/captcha?f=business_reg&tmp='+Math.random()});
			...
			$.post("/site/reg1",c,function(e){
				...
				if(e.error=="captcha"){
					...
					if(e.error=="username"){
						...
						if(e.error=='success'){
							//alert("注册成功");
							window.location.href="/wechat/main";
						}else{
							alert(e.error)
						}
					}
